...
- **Start popover tabs (Start now / Get link):** the Start popover (`FormLibraryRow.tsx`) has two tabs sharing the same option state. **Start now** starts immediately; **Get link** shows a live URL preview (query params rendered as chips that flash on change), a plain-English summary (session mode/duration · late sync · live feedback), and a **Copy link** CTA (sonner toast with the summary). "Copy start link…" in the ⋯ menu opens the popover directly on Get link with a one-time attention pulse — no more copying with invisible settings. Popover kind lives in `DashboardFormLibrary`'s `openPopover` state (`"start" | "start-link" | "menu"`, exported type `FormLibraryPopover`).
...
- **Redesigned chrome:** autosave indicator (debounced ~800ms), collapsible form details, summary strip (count / points / auto-grade), identity question cards with summary tokens that expand image/response/scoring panels, between-card insert, categorized **Add question** picker (`components/builder/BuilderTypePicker.tsx`). Live teacher feedback moved to the form-library Start popover.
- **Builder view switch (Editing ↔ Student view):** persistent segmented control in the builder header (same position in both modes, sliding thumb, `V` keycap hint; CSS `tp-view-switch` in `app/globals.css`). Keyboard: `V` toggles (outside inputs), `Escape` exits student view. Student view shows a sticky preview context bar (`tp-preview-bar`: pulsing dot, "answers aren't saved" copy, **Reset answers** clearing `previewAnswers`, **Back to editing**); the exam header is non-sticky in preview (`tp-exam-shell--preview`). Each builder question card reveals a **Preview here** ghost pill on hover/focus (`tp-builder-peek`) that switches to student view, scrolls to that question (`window.scrollTo` offset −110px), and highlights the card ~1.6s (`data-preview-hilite`). State stays on the existing `mode` / `isBuilderStudentPreview` / `previewAnswers` machinery in `HomeClient.tsx`.
...
*Last reviewed: 2026-08-23 (builder edit ↔ student-view segmented switch with per-question preview; Start popover Get link tab with live URL preview).*
