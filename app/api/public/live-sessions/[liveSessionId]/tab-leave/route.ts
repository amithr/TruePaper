...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    const { error } = await supabase.rpc("suspend_live_session_student_tab_leave", {
    ...
    if (error) {
      if (error.message.includes("suspend_live_session_student_tab_leave") || error.code === "42883") {
        return NextResponse.json(
          {
            error:
              "Database is missing suspend_live_session_student_tab_leave (3-arg) or it is outdated. Run migration 20260424130000_live_student_display_name.sql.",
          },
          { status: 503 },
        );
      }
